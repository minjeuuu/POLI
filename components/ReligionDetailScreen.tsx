...
import { ArrowLeft, Globe, Map as MapIcon, Users, FileText, Sparkles, AlertCircle, BookOpen, Clock, Fingerprint, Compass, ShieldAlert, Zap, Search, Printer, Flame, Building2 } from 'lucide-react';
...
<div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8">
                ...
                <div className="text-center space-y-6">
                    ...
                    <h1 className="text-2xl md:text-4xl font-serif font-extrabold text-stone-900 dark:text-white tracking-tight">{data.name}</h1>
                ...
                <div className="flex flex-col gap-8">
                    ...
                    <div className="space-y-8">
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black print:shadow-none">
                            ...
                            <ul className="space-y-2">
                                {(Array.isArray(data.deities) ? data.deities : []).map((d: string, i: number) => <li key={i} className="font-serif text-lg">{d}</li>)}
                            </ul>
                        </div>
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black print:shadow-none">
                            ...
                            <ul className="space-y-2 text-sm italic font-serif text-stone-600 dark:text-stone-300">
                                {(Array.isArray(data.sacredTexts) ? data.sacredTexts : []).map((d: string, i: number) => <li key={i}>{d}</li>)}
                            </ul>
                        </div>
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black print:shadow-none">
                        ...
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black print:shadow-none">
                           ...
                           <ul className="space-y-2 font-serif">
                                {(Array.isArray(data.founders) ? data.founders : []).map((d: string, i: number) => <li key={i}>{d}</li>)}
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className=" space-y-8">
                        <div className="bg-white dark:bg-stone-900 p-8 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black print:shadow-none">
                            <h3 className="text-xl font-serif font-bold border-b border-academic-line dark:border-stone-800 pb-2 text-stone-900 dark:text-white mb-6 border-b border-stone-100 dark:border-stone-800 pb-4">Core Tenets & Philosophy</h3>
                            <ul className="space-y-4">
                                {(Array.isArray(data.coreTenets) ? data.coreTenets : []).map((t: string, i: number) => (
                        ...
                        <div className="bg-white dark:bg-stone-900 p-8 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black print:shadow-none">
                            <h3 className="text-xl font-serif font-bold border-b border-academic-line dark:border-stone-800 pb-2 text-stone-900 dark:text-white mb-6 border-b border-stone-100 dark:border-stone-800 pb-4">Rituals & Practices</h3>
                            <ul className="space-y-4">
                                {(Array.isArray(data.rituals) ? data.rituals : []).map((t: string, i: number) => (
                                    <li key={i} className="flex font-serif text-lg leading-relaxed items-start gap-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                            ...
                            <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {(Array.isArray(data.sects) ? data.sects : []).map((s: string, i: number) => (
