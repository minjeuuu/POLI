...
import { ArrowLeft, Flame, Building2, Map as MapIcon, Users, FileText, Sparkles, AlertCircle, BookOpen, Clock, Fingerprint, Compass, ShieldAlert, Zap, Search, Printer, Eye, Lock } from 'lucide-react';
                ...
                <div className="flex flex-col gap-8">
                    ...
                    <div className="space-y-8">
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black print:shadow-none">
                            ...
                            <ul className="space-y-2">
                                {(Array.isArray(data.keyFigures) ? data.keyFigures : []).map((d: string, i: number) => <li key={i} className="font-serif text-md">{d}</li>)}
                            </ul>
                        </div>
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black print:shadow-none">
                            ...
                            <ul className="space-y-2 text-sm font-serif text-stone-600 dark:text-stone-300">
                                {(Array.isArray(data.coreFocus) ? data.coreFocus : []).map((d: string, i: number) => <li key={i}>• {d}</li>)}
                            </ul>
                        </div>
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black print:shadow-none">
                    ...
                    <div className=" space-y-8">
                        ...
                        <div className="bg-academic-paper dark:bg-stone-800/50 p-8 rounded-2xl border border-academic-line dark:border-stone-700 print:bg-stone-100 print:border-black">
                            <h3 className="text-xl font-serif font-bold border-b border-academic-line dark:border-stone-800 pb-2 text-stone-900 dark:text-white mb-6 flex items-center gap-3"><Eye className="w-6 h-6 text-stone-500" /> Function & Historical Impact</h3>
                        ...
                        <div className="bg-white dark:bg-stone-900 p-8 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black print:shadow-none">
                            <h3 className="text-xl font-serif font-bold border-b border-academic-line dark:border-stone-800 pb-2 text-stone-900 dark:text-white mb-6 border-b border-stone-100 dark:border-stone-800 pb-4">Declassified Major Operations</h3>
                            <ul className="space-y-4">
                                {(Array.isArray(data.majorOperations) ? data.majorOperations : []).map((t: string, i: number) => (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                            ...
                            <div className="bg-stone-100 dark:bg-stone-800 p-6 rounded-2xl border border-stone-300 dark:border-stone-700 shadow-sm print:border-black">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {(Array.isArray(data.capabilities) ? data.capabilities : []).map((s: string, i: number) => (
