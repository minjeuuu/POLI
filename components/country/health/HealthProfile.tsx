...
export const HealthProfile = ({ data, onNavigate }: { data: any, onNavigate?: (type: string, payload: any) => void }) => {
    if (!data) return null;
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white dark:bg-stone-900 p-6 rounded-xl border border-stone-200 dark:border-stone-800 flex items-center gap-4">
                ...
                <div>
                    ...
                    <div className="text-2xl font-bold text-stone-800 dark:text-stone-100">{data.lifeExpectancy }</div>
                </div>
            </div>
            <div className="bg-white dark:bg-stone-900 p-6 rounded-xl border border-stone-200 dark:border-stone-800 flex items-center gap-4">
                ...
                <div>
                    ...
                    <div className="text-lg font-bold text-stone-800 dark:text-stone-100">{data.healthcareSystem }</div>
                </div>
            </div>
            <div className="bg-white dark:bg-stone-900 p-6 rounded-xl border border-stone-200 dark:border-stone-800 md:col-span-2">
                ...
                <div className="flex flex-wrap gap-2">
                    {data.majorDiseases?.map((item: string, i: number) => (
                        <span key={i} onClick={() => onNavigate && onNavigate('Concept', item)} className="cursor-pointer hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors px-3 py-1 bg-red-50 dark:bg-red-900/10 text-red-600 dark:text-red-400 rounded-full text-xs font-bold border border-red-100 dark:border-red-900/20">{item}</span>
                    ))}
                </div>
            </div>
             <div className="bg-white dark:bg-stone-900 p-6 rounded-xl border border-stone-200 dark:border-stone-800 md:col-span-2 flex items-center justify-between">
                ...
                <div className="font-mono text-lg font-bold text-stone-800 dark:text-stone-100">{data.expenditure }</div>
            </div>
        </div>
    );
};
