        ...
        <div className="space-y-5">
            ...
            {tab==='safety' && d.safety && <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Sec title="Safety for Tourists" icon={<Shield className="w-4 h-4 text-green-500"/>}>
                    ...
                    {d.safety.commonScams?.length>0&&<div className="mt-3"><div className="text-[10px] font-bold uppercase text-stone-400 mb-2">Common Scams</div>{d.safety.commonScams.map((s:string,i:number)=><div key={i} className="text-xs py-0.5 text-stone-600 dark:text-stone-400">! {s}</div>)}</div>}
