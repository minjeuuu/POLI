...
export const ReligiousBreakdown: React.FC<{ data: DemographicsProfile, onNavigate?: (type: string, payload: any) => void }> = ({ data, onNavigate }) => {
        ...
        <div className="bg-white dark:bg-stone-900 p-6 rounded-xl border border-stone-200 dark:border-stone-800">
            ...
            <div className="space-y-3">
                {data.religions.map((rel, i) => (
                    <div key={i} className="relative group cursor-pointer" onClick={() => onNavigate && onNavigate('Concept', rel.name)}>
                        <div className="flex justify-between text-sm mb-1 z-10 relative">
                            <span className="font-serif text-stone-700 dark:text-stone-300 group-hover:text-academic-accent transition-colors">{rel.name}</span>
                            <span className="font-bold text-academic-accent dark:text-indigo-400">{rel.percentage}</span>
                        </div>
                        <div className="w-full h-1.5 bg-stone-100 dark:bg-stone-800 rounded-full overflow-hidden">
                            <div 
                                className="h-full bg-academic-gold/50 dark:bg-indigo-500/50 group-hover:bg-academic-accent transition-colors" 
