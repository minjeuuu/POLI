        ...
        <div className="space-y-5">
            ...
            {tab==='traditions' && d.traditions && <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Sec title="Cultural Customs" icon={<Users className="w-4 h-4 text-amber-500"/>}>
                    ...
                    {d.traditions.taboos?.length>0&&<div className="mt-3"><div className="text-[10px] font-bold uppercase text-stone-400 mb-2">Cultural Taboos</div>{d.traditions.taboos.map((t:string,i:number)=><div key={i} className="text-xs py-0.5 text-red-600 dark:text-red-400">! {t}</div>)}</div>}
                </Sec>
            </div>}
