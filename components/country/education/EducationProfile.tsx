...
export const EducationProfile = ({ data, onNavigate }: { data: any, onNavigate?: (type: string, payload: any) => void }) => {
    if (!data) return null;
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white dark:bg-stone-900 p-6 rounded-xl border border-stone-200 dark:border-stone-800 flex items-center gap-4">
                ...
                <div>
                    ...
                    <div className="text-2xl font-bold text-stone-800 dark:text-stone-100">{data.literacyRate }</div>
                </div>
            </div>
            <div className="bg-white dark:bg-stone-900 p-6 rounded-xl border border-stone-200 dark:border-stone-800 flex items-center gap-4">
                ...
                <div>
                    ...
                    <div className="text-lg font-bold text-stone-800 dark:text-stone-100">{data.system }</div>
                </div>
            </div>
            <div className="bg-white dark:bg-stone-900 p-6 rounded-xl border border-stone-200 dark:border-stone-800 md:col-span-2">
                ...
                <ul className="space-y-2">
                    {data.universities?.map((item: string, i: number) => (
                        <li key={i} onClick={() => onNavigate && onNavigate('Concept', item)} className="flex items-center gap-2 text-sm text-stone-600 dark:text-stone-300 cursor-pointer hover:text-academic-accent transition-colors">
