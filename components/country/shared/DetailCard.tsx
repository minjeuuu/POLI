...
export const DetailCard: React.FC<DetailCardProps> = ({ label, value, icon: Icon, subValue, onClick }) => {
    if (!value || value === "Unknown" || value === "N/A" || value === "unavailable") return null;
    return (
        <div 
            onClick={onClick}
        ...
        >
            ...
            {subValue && !String(subValue).includes('undefined') && <div className="text-xs text-stone-500 mt-1 font-mono">{subValue}</div>}
        </div>
    );
};
