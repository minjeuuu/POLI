...
import { ArrowLeft, Scale, Users, FileText, Anchor, BookOpen, Clock, Fingerprint, Compass, Zap, Search, Printer, TriangleAlert, Building2, Eye, Flag, MapPin } from 'lucide-react';
...
<div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8">
                ...
                <div className="text-center space-y-6">
                    ...
                    <div className="flex items-center justify-center flex-wrap gap-4 text-sm font-bold uppercase tracking-widest text-stone-500 dark:text-stone-400">
                        ...
                        {data.year && <span className="flex items-center gap-1"><Clock className="w-4 h-4" /> {data.year}</span>}
                        <span className="hidden sm:inline">•</span>
                        {data.country && <span className="flex items-center gap-1"><MapPin className="w-4 h-4" /> {data.country}</span>}
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-8">
                    ...
                    <div className="space-y-8">
                        <div className="bg-blue-50 dark:bg-blue-900/10 p-6 rounded-2xl border border-blue-200 dark:border-blue-900/30 print:border-black print:bg-white print:border-solid">
                            ...
                            <ul className="space-y-4">
                                {(Array.isArray(data.candidates) ? data.candidates : []).map((d: any, i: number) => (
                        ...
                        <div className="bg-white dark:bg-stone-900 p-6 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm print:border-black">
                            ...
                            <ul className="space-y-2 text-sm font-serif text-stone-600 dark:text-stone-300">
                                {(Array.isArray(data.keyIssues) ? data.keyIssues : []).map((d: string, i: number) => <li key={i}>• {d}</li>)}
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className=" space-y-8">
                        ...
                        <div className="bg-academic-paper dark:bg-stone-800/50 p-8 rounded-2xl border border-academic-line dark:border-stone-700 print:bg-stone-100 print:border-black">
                            <h3 className="text-xl font-serif font-bold border-b border-academic-line dark:border-stone-800 pb-2 text-stone-900 dark:text-white mb-6 flex items-center gap-3"><Building2 className="w-6 h-6 text-stone-500" /> Significance & Geopolitics</h3>
