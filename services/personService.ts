...
export const fetchPersonDetail = async (name: string): Promise<PersonDetail> => {
    const cacheKey = `person_poli_v2_search_${name.replace(/\s+/g, '_')}`;
    ...
    return withCache(cacheKey, async () => {
        ...
        try {
            ...
            ${wikiContext ? `WIKIPEDIA EXTRACT (Use this data to assure accuracy): ${wikiContext.substring(0, 1500)}` : ""}
            ...
            **DIRECTIVES:**
            ...
            return { 
                ...FALLBACK_PERSON, 
                name,
                bio: wikiContext || FALLBACK_PERSON.bio,
                imageUrl: wikiImage || FALLBACK_PERSON.imageUrl
            }; 
        }
    });
};
